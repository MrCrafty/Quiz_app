...
const props = defineProps({
  closeeditServiceModal: Function,
  serviceId: Number,
});
...
axios
  .post(
    ...
    JSON.stringify({ service_id: props.serviceId, name: "harsh" }),
    {
      headers: {
  ...
  .then((data) => {
    serviceDescription.value = data.data.data.service_description;
    serviceName.value = data.data.data.service_name;
    basePrice.value = data.data.data.price;
    timeRequired.value = data.data.data.time_required;
  });
const handleSubmit = async (e) => {
  e.preventDefault();
  await axios
    .post(
      "http://localhost:5000/api/editService",
      JSON.stringify({
  ...
  <div class="login">
    <div class="login-modal bg-dark">
      <button
        ...
        @click="props.closeeditServiceModal"
      ></button>
      <h3 class="my-4 fs-1 text-white">Edit Service</h3>
